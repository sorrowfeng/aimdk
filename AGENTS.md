...
│       ├── py_examples/
│       │   ├── ai_voice_chat.py      # 百度 ASR + Codex AI 问答
...
DISPLAY=dummy:0 SSH_ASKPASS=/tmp/askpass.sh scp \
  ...
  agi@172.16.20.110:/home/agi/aimdk/src/examples/src/vr_teleop/vr_teleop_node.cpp \
  </dev/null
...
- 百度在线 ASR → 唤醒词过滤 → Codex AI（`Codex-sonnet-4-6` via Kimi API）→ TTS 播报
